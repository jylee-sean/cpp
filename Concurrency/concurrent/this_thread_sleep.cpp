...
// sleep for : 주어진 시간 만큼 현재 스레드 멈추기(재우기)
// sleep until: 주어진 시간 까지 현재 스레드 멈추기(재우기)
int main()
{
    std::this_thread::sleep_for(std::chrono::seconds(3)); //
    std::this_thread::sleep_for(3s); // 3ms, 3ns, 3min    // literals 열어줘야함
    //std::this_thread::sleep_for(3);  // error 

    std::chrono::time_point tp1 = std::chrono::steady_clock::now();//현재시간을 타임 포인터
