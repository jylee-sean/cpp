...
HRESULT CModule1::SetObjStateSO()
{
	...
	for (auto motor = motors.begin(); motor != motors.end(); motor++) {
		m_Outputs.mode = motor->GetMode();
		m_Outputs.acc = motor->GetAcceleration();
		m_Outputs.dec = motor->GetDeceleration();
		m_Outputs.vel = motor->GetVelocity();
	}
...
HRESULT CModule1::CycleUpdate(ITcTask* ipTask, ITcUnknown* ipCaller, ULONG_PTR context)
{
	...
	switch (m_DataArea3.state)
	{
		case state::servo_off:
			servo_off();
			break;
		case state::servo_on:
			servo_on();
