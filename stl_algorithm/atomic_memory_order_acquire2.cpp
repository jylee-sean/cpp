...
/* is_ready.store(true, std::memory_order_release) 과 
   while (!is_ready.load(std::memory_order_acquire)) 간의 동기화

*/

// memory_order_acq_rel: acquire & release 모두 수행 (읽기 쓰기를 모두 수행하는 명령(fetch_add)들에서 사용)
